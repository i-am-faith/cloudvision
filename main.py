...
if selected_page == "Home":
    ...
    try:
        if place:
            # Geocode the entered place to get its coordinates
            geolocator = Nominatim(user_agent="weather_app")
            location = geolocator.geocode(place)
            if location:
                latitude, longitude = location.latitude, location.longitude

                # Display map showing the entered location
                st.subheader(f"Location of {place}:")
                ...
                st.warning("Invalid location! Please enter a valid city.")
    except KeyError:
